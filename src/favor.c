...
int pick_any_token_in_market(int current_player, struct player players[NB_PLAYERS], int a, struct market* market) { //(int a) is used in the fonction pick_token in game.c 
    ...
    if (market->nbr_token>0) {
        ...
        pick_tokens(current_player, players, a, market);
        return 1;
    }
    return 0;
}
