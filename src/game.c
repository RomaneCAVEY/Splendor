...
void pay(struct player players[NB_PLAYERS], int index, int current){
	...
	struct builder_t *builder= guild_builder_in_guild(index);
	 skill skill_panic_guild=builder_has_the_power_i(builder,1);
	 skill skill_master_hands=builder_has_the_power_i(builder,4);
	void* ressource=guild_builder_in_guild(index);
	skill_panic_guild(current, players, ressource);
	skill_master_hands(current, players, ressource);
...
void pick_tokens(int current_player, struct player players[NB_PLAYERS], int a){
	...
	skill_panic_market( current_player, players, ressource);
	skill_token_steal( current_player, players, ressource);
